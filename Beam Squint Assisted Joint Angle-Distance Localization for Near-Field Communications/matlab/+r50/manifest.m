...
%MANIFEST Hash R50 code and executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r50", "+r49", "+r48", "+r47", "+r46", "+r45", ...
...
listing = [dir(fullfile(project, "experiments", "*round50*.m")); ...
    dir(fullfile(project, "tests", "round50*.m"))];
for index = 1:numel(listing)
    file = string(fullfile(listing(index).folder, listing(index).name));
    relative = extractAfter(file, strlength(project)+1);
    paths(end+1, 1) = replace(relative, string(filesep), "/"); %#ok<AGROW>
