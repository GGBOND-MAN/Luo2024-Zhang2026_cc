function output = run_round41_schemeG_final_shard(shardId, options)
%RUN_ROUND41_SCHEMEG_FINAL_SHARD Run one future authorized 700-row shard.
...
folder = r41.assertExecutionTarget(project, shardId, options.Resume);
...
identity = struct( ...
    version="R41-independent-final-shard-v1", ...
    ...
    authorizationDigest=authorizationDigest, authorized=true, ...
...
% All authorization and identity checks above precede observation generation.
...
for first = 1:options.BatchSize:numel(pending)
    ...
    fprintf("R41 final shard %d/2: %d/%d\n", shardId, ...
...
fprintf("ROUND41_FINAL_SHARD_COMPLETE shard=%d rows=%d failures=%d\n", ...
