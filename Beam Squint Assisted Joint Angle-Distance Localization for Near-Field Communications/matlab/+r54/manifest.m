...
%MANIFEST Hash R54 code and frozen R53 executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r54", "+r53", "+r51", "+r50", "+r45", ...
...
listing = [dir(fullfile(project, "experiments", "*round54*.m")); ...
    dir(fullfile(project, "tests", "round54*.m"))];
for index = 1:numel(listing)
    file = string(fullfile(listing(index).folder, listing(index).name));
    relative = extractAfter(file, strlength(project)+1);
    paths(end+1, 1) = replace(relative, string(filesep), "/"); %#ok<AGROW>
