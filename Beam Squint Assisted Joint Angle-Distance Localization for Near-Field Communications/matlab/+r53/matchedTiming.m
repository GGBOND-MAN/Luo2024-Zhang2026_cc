...
%MATCHEDTIMING Compare standalone P_FALF and C_enhanced runtime.
...
if height(timingDesign) ~= numel(protocol.timing.positionIds)
    error("r53:TimingDesignMismatch", ...
        "The R53 timing rows are not unique and complete.");
end
...
candidate = summary(summary.method == "P_FALF", :);
reference = summary(summary.method == "C_enhanced", :);
comparison = table("P_FALF", "C_enhanced", ...
...
output = struct(version="R53-matched-standalone-timing-v1", ...
...
if method == "P_FALF"
    estimate = r53.estimatePFALF(cfg, replay.observation, ...
