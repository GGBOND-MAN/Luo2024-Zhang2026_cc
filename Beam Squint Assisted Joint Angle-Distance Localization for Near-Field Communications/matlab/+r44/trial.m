...
%TRIAL Execute one paired R44 development row.
...
try
    ...
    if orderIndex == 1
        ...
        h = r44.estimate(cfg, replay.observation, replay.snapshots, ...
            scan, protocol);
    elseif orderIndex == 2
        order = "C-H-P_A";
        c = runBaseline("C_enhanced");
    ...
    result.methodNames = ["P_A", "C_enhanced", ...
        "H_seqZ", "H_seqVP", "F_L06"];
    result.thetaDeg = [pa.thetaDeg, c.thetaDeg, ...
        h.H_seqZ.thetaDeg, h.H_seqVP.thetaDeg, front.thetaDeg];
    result.rangeM = [pa.rangeM, c.rangeM, ...
        h.H_seqZ.rangeM, h.H_seqVP.rangeM, front.rangeM];
    result.runtimeSeconds = [pa.cost.totalOnlineSeconds, ...
        c.cost.totalOnlineSeconds, commonH+h.cost.zProfileSeconds, ...
        commonH+h.cost.vpProfileSeconds, h.cost.frontSeconds];
    result.h = h;
    result.pa = pa;
    result.c = c;
catch exception
