...
%SOURCEMANIFEST Hash R55 code and all executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r55", "+r54", "+r53", "+r51", "+r50", ...
    "+r45", "+r42", "+r41", "+r40", "+r38", "+r37", ...
...
listing = [dir(fullfile(project, "experiments", "*round55*.m")); ...
    dir(fullfile(project, "tests", "round55*.m"))];
for index = 1:numel(listing)
    file = string(fullfile(listing(index).folder, listing(index).name));
    relative = extractAfter(file, strlength(project)+1);
    paths(end+1, 1) = replace(relative, string(filesep), "/"); %#ok<AGROW>
