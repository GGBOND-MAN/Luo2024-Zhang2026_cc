...
%MANIFEST Hash R53 code and executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r53", "+r51", "+r50", "+r45", "+r42", ...
...
listing = [dir(fullfile(project, "experiments", "*round53*.m")); ...
    dir(fullfile(project, "tests", "round53*.m"))];
for index = 1:numel(listing)
    file = string(fullfile(listing(index).folder, listing(index).name));
    relative = extractAfter(file, strlength(project)+1);
    paths(end+1, 1) = replace(relative, string(filesep), "/"); %#ok<AGROW>
