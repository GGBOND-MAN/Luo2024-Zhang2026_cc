function run_round30_pilot_shard(shardId, shardCount, options)
%RUN_ROUND30_PILOT_SHARD Evaluate six frozen candidates on the 60 pilot users.
...
if shardId > shardCount
    error("r30:InvalidShard", "shardId must not exceed shardCount.");
end
...
for first = 1:options.BatchSize:numel(pending)
    ...
    parfor index = 1:numel(taskRows)
        batch{index} = r30.screenRow( ...
            cfg, scan, protocol, candidates, batchDesign(index, :));
    end
    results(taskRows) = batch;
    save(checkpoint, "identity", "results", "environment", "-v7.3");
    fprintf("R30 pilot shard %d: %d/%d session %.2f h\n", ...
...
fprintf("ROUND30_PILOT_SHARD_COMPLETE failures=%d\n", nnz(failed));
end
