...
%MANIFEST Hash Round31 sources and all inherited executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r31", "+r30", "+r29", "+fsjad", "+jad", "algorithms"];
paths = strings(0, 1);
for folder = folders
    listing = dir(fullfile(project, folder, "**", "*.m"));
    paths = appendListing(paths, listing, project);
end
listing = [dir(fullfile(project, "experiments", "*round31*.m")); ...
    dir(fullfile(project, "tests", "round31*.m"))];
paths = appendListing(paths, listing, project);
