...
%MANIFEST Hash R45 sources and executable dependencies.

arguments
    project (1, 1) string
end

folders = ["+r45", "+r41", "+r40", "+r38", "+r37", ...
...
listing = [dir(fullfile(project, "experiments", "*round45*.m")); ...
    dir(fullfile(project, "tests", "round45*.m"))];
for index = 1:numel(listing)
    file = string(fullfile(listing(index).folder, listing(index).name));
    relative = extractAfter(file, strlength(project)+1);
    paths(end+1, 1) = replace(relative, string(filesep), "/"); %#ok<AGROW>
